...
describe('auth store', () => {
  ...
  describe('login', () => {
    it('成功后持久化 token 与 user 到 localStorage', async () => {
  ...
  describe('store 初始化', () => {
    it('从 localStorage 恢复 user（页面刷新场景）', () => {
      localStorage.setItem('token', 'tok')
      localStorage.setItem('user', JSON.stringify(mockUser))
      const store = useAuthStore()
      expect(store.token).toBe('tok')
  ...
  describe('fetchProfile', () => {
    ...
    it('失败（token 无效）时自动登出并清理 localStorage', async () => {
      localStorage.setItem('token', 'tok')
      localStorage.setItem('user', JSON.stringify(mockUser))
  ...
  describe('bootstrapSession', () => {
    it('SSO cookie 引导成功后持久化 token 与 user', async () => {
      ...
      expect(localStorage.getItem('token')).toBe('sso-tok')
      expect(JSON.parse(localStorage.getItem('user') || 'null')).toEqual(mockUser)
  ...
  describe('logout', () => {
    it('清理 token 与 user 的 localStorage', () => {
      localStorage.setItem('token', 'tok')
      localStorage.setItem('user', JSON.stringify(mockUser))
      const store = useAuthStore()
      store.logout()
