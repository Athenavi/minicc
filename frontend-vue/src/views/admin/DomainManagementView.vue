        ...
        :columns="[
          ...
          { title: 'SSL 状态', key: 'ssl_status', width: 120,
            customRender: ({ record }: any) => (
              <Tag color={getSSLStatusColor(record.ssl_status)}>
                {record.ssl_status === 'active' ? '有效' : 
          ...
          { title: '自动续期', key: 'auto_renew', width: 100,
            customRender: ({ record }: any) => (
              <Tag color={record.auto_renew ? 'green' : 'default'}>
                {record.auto_renew ? '开启' : '关闭'}
              </Tag>
            )
          },
          { title: '状态', key: 'status', width: 100,
            customRender: ({ record }: any) => (
              <Tag color={getDomainStatusColor(record.status)}>{record.status}</Tag>
            )
          },
          { title: '操作', key: 'actions', width: 280, fixed: 'right',
            customRender: ({ record }: any) => (
              <Space>
                <Button size="small" icon={<CheckCircleOutlined />} onClick={() => verifyDNS(record.id)}>验证 DNS</Button>
                <Button size="small" icon={<SyncOutlined />} onClick={() => renewSSL(record.id)}>续期 SSL</Button>
                <Button size="small" icon={<ReloadOutlined />} onClick={() => { selectedDomain.value = record; currentForm.value = record; modalVisible.value = true; }}>编辑</Button>
              </Space>
            )
          }
        ]"
