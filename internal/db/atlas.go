...
func RunAtlasMigrations(ctx context.Context, pool *pgxpool.Pool, dir string) error {
	...
	for _, v := range versions {
		...
		var count int
		pool.QueryRow(ctx, "SELECT COUNT(*) FROM schema_migrations WHERE version = $1", parseVersion(v)).Scan(&count)
		if count > 0 {
			continue
		}
...
func EnsureDatabase(ctx context.Context, dsn string) error {
	...
	// We need to connect to the 'postgres' default database and CREATE DATABASE if needed
	defaultDSN := strings.SplitN(dsn, "/", 2)
	if len(defaultDSN) != 2 {
		return nil // can't parse, skip
	}
	adminDSN := defaultDSN[0] + "/postgres?" + strings.SplitN(defaultDSN[1], "?", 2)[1]
	...
	// Extract target DB name
	dbName := strings.SplitN(defaultDSN[1], "?", 2)[0]
