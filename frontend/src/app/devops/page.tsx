  ...
  const runTool = async (name: string) => {
    ...
    try {
      const resp = await fetch("http://localhost:8000/api/submit", {
        ...
        body: JSON.stringify({ message: `/tools ${name}`, session_id: "devops" }),
      });
        ...
        <div className="flex gap-1 mb-6 bg-white dark:bg-gray-800 rounded-lg p-1 shadow-sm overflow-x-auto">
          {tabs.map((t) => (
            <button key={t.id} onClick={() => setActiveTab(t.id)}
              className={`flex-1 px-4 py-2.5 rounded-md text-sm font-medium whitespace-nowrap transition-all ${
            ...
            >
              ...
              <span className="block text-xs opacity-70 font-normal">{t.desc}</span>
            </button>
          ))}
        </div>
